...
import { TrendingUp, TrendingDown, Users, CalendarDays, Target, Award, AlertTriangle } from "lucide-react";
import { cn } from "@/lib/utils";

interface StatCard { label: string; value: string; delta?: string; up?: boolean; icon: React.ComponentType<{ className?: string }>; }

interface PlayerStat {
  name: string; position: string;
  fielding: number; hitting: number; throwing: number;
  attendance: number; flagged?: boolean;
}
...
function SkillBar({ value, color = "bg-emerald-500" }: { value: number; color?: string }) {
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="text-[11px] text-muted-foreground w-7 text-right">{value}</span>
    </div>
  );
}
...
export function Analytics() {
  return (
    <div className="space-y-5">
      <div>
        <h2 className="font-heading text-xl text-foreground">Team Analytics</h2>
        <p className="text-sm text-muted-foreground mt-0.5">Spring 2026 · April snapshot</p>
      </div>
        ...
        {STAT_CARDS.map((s, i) => (
          ...
          >
            <div className="flex items-start justify-between">
              ...
              {s.delta && (
                <div className={cn("flex items-center gap-0.5 text-[11px] font-medium", s.up ? "text-emerald-600" : "text-red-500")}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="dos-card p-5">
          <h3 className="font-heading text-sm font-bold text-foreground mb-4">Weekly Attendance</h3>
          <div className="flex items-end gap-2 h-28">
            {WEEKLY.map((w) => (
              <div key={w.week} className="flex-1 flex flex-col items-center gap-1">
                ...
                <span className="text-[10px] font-medium text-foreground">{w.pct}%</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="lg:col-span-2 dos-card p-5">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-heading text-sm font-bold text-foreground">Player Skill Breakdown</h3>
            <div className="flex items-center gap-3 text-[11px] text-muted-foreground">
              ...
              <span className="flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-blue-500 inline-block" />Hitting</span>
              <span className="flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-amber-500 inline-block" />Throwing</span>
            </div>
          </div>
            ...
            {PLAYERS.map((p, i) => (
              ...
              >
                <div className="flex items-center gap-2 min-w-0">
                  {p.flagged && <AlertTriangle className="h-3 w-3 text-red-500 shrink-0" />}
                ...
                <SkillBar value={p.hitting}   color="bg-blue-500" />
                <SkillBar value={p.throwing}  color="bg-amber-500" />
                <div className="text-[11px] text-muted-foreground text-right whitespace-nowrap">{p.attendance}%</div>
              </motion.div>
            ))}
          </div>
          <div className="mt-4 pt-3 border-t flex justify-between text-[10px] text-muted-foreground/60">
